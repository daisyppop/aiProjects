...
# 아래 CONTENT 리스트는 앱 내부 데이터베이스 역할입니다.
# 새로운 콘텐츠를 추가하거나 기존 데이터를 수정하려면 이곳에 항목을 추가
#
# 1) CONTENT에 항목을 추가할 때 반드시 아래 키를 포함해야 합니다:
#    - title: 콘텐츠 제목
#    - genres: 장르 목록 (예: ["액션", "스릴러"])
#    - rating: 0~5 사이 숫자 평점
#    - year: 공개 또는 제작 연도
#    - desc: 설명 텍스트
#    - badge: 배지 텍스트 (예: "🔥 HOT", "⭐ TOP")
#    - color: 배지/태그 색상 코드
#    - image: 이미지 파일 경로 (예: "images/squidgame.png")
#
# 2) image 경로 작성 규칙:
#    - 현재 프로젝트 폴더를 기준으로 한 상대 경로입니다.
#    - 실제 이미지 파일은 반드시 images 폴더 안에 있어야 합니다.
#    - 추천하는 형식: 영문+숫자+언더스코어 (예: images/new_show.png)
#    - 한글이나 공백, 특수문자는 파일명에서 오류를 유발할 수 있으니 주의하세요.
#
# 3) genres 값은 GENRES 목록에 있는 이름과 일치시키면 자동으로 색상이 적용됩니다.
#    - 예: "스릴러", "드라마", "액션"
#    - 목록에 없는 장르를 쓰면 기본 색상 #444가 사용됩니다.
#
# 4) 새로운 항목 추가 시 실제 이미지 파일을 images 폴더에 넣어야 합니다.
#    - 예: 이미지 파일이 images/newseries.png라면
#      "image": "images/newseries.png"로 입력하세요.
#
# 5) CONTENT 리스트에 데이터를 추가하면 앱을 재실행할 때 자동으로 반영됩니다.
#    - 별도 DB 수정 작업이 없어도 리스트에 항목을 추가하면 추천 화면에 표시됩니다.
#
# 예시 항목:
#    {"title": "새로운 콘텐츠", "genres": ["액션", "SF"], "rating": 4.7, "year": 2025,
#     "desc": "새로운 오리지널 시리즈 설명입니다.",
#     "badge": "🔥 NEW", "color": "#00BFFF",
#     "image": "images/newseries.png"},
#
# 6) 만약 나중에 실제 데이터베이스를 추가하면,
#    이 CONTENT 리스트 구조를 그대로 유지하면 코드에서 읽어오기 쉽게 확장할 수 있습니다.
CONTENT = [
...
class RecommendScreen(tk.Frame):
    ...
    # ── 추천 알고리즘 ───────────────────────────────────────────
    # 1) 사용자가 선택한 장르와 콘텐츠 장르 간의 교집합 크기(match)를 기준으로
    #    기본 점수를 계산합니다.
    #    - match가 높을수록 추천 우선순위가 올라갑니다.
    #    - 장르 매칭은 사용자의 취향과 콘텐츠 관련성을 가장 크게 반영하도록 설계했습니다.
    #
    # 2) 평점(rating)을 추가 가중치로 반영합니다.
    #    - 높은 평점을 받은 콘텐츠는 같은 장르 매칭 수일 때 더 먼저 추천됩니다.
    #    - 평점은 0~5 사이 값이며, 5점 만점 콘텐츠가 더 높게 평가됩니다.
    #
    # 3) randomness(무작위성)로 동일한 점수인 콘텐츠 간 중복 추천을 완화합니다.
    #    - 추천 결과가 항상 완전히 동일한 순서로 나오지 않도록 합니다.
    #    - 동일한 match와 rating을 가진 콘텐츠도 약간의 변동을 주어 다양성을 확보합니다.
    #
    # 4) 결과 정렬 후 상위 8개를 반환합니다.
    #    - 한 번에 보여줄 카드 수를 제한하여 UI가 과도하게 길어지지 않게 합니다.
    #
    # 5) 만약 사용자가 선택한 장르와 하나도 매칭되지 않는 경우,
    #    장르 기반 추천 대신 전체 콘텐츠를 평점 순으로만 정렬합니다.
    #    - 사용자 선택이 적거나 특이한 조합일 때도 추천 목록을 제공합니다.
    def _score_content(self):
        # 선택된 장르 정보를 set으로 변환하여 intersection 연산을 빠르게 수행합니다.
        sel = set(self.selected)
        scored = []

        for c in CONTENT:
            # 콘텐츠가 가진 장르와 사용자가 선택한 장르의 교집합 개수를 계산합니다.
            # 예: 사용자가 ['액션', '스릴러']를 선택하고 콘텐츠가 ['스릴러','드라마']라면 match=1.
            match = len(set(c["genres"]) & sel)

            # 장르가 하나도 겹치지 않으면 추천 우선순위에서 제외합니다.
            if match == 0:
                continue

            # 추천 점수 계산식:
            #   장르 매칭 개수 × 10 + 평점 × 5 + 임의 노이즈
            # 이유: 장르 매칭이 가장 중요하지만, 평점도 추천 품질에 큰 영향을 줍니다.
            score = match * 10 + c["rating"] * 5 + random.uniform(0, 3)

            # score 값을 콘텐츠 정보에 추가하여 이후 정렬에 활용합니다.
            scored.append({**c, "score": score, "match": match})

        # 점수가 높은 순으로 정렬(내림차순).
        scored.sort(key=lambda x: -x["score"])

        # 추천 후보가 하나도 없는 경우: 장르 기준 필터를 해제하고 평점 기준으로 정렬.
        if not scored:
            scored = sorted(CONTENT,
                            key=lambda x: -(x["rating"] + random.uniform(0, 1)))

        # 화면에 표시할 최대 추천 개수는 8개로 제한합니다.
        return scored[:8]
